...
export class BasketData implements IBasket {
  ...
  addItem(product: IProduct): void {
    ...
    this.events.emit('basket:updated', { items: this._items, totalAmount: this._totalAmount });
  }
  ...
  removeItem(product: IProduct): void {
    ...
    this.updateTotalAmount();
    this.events.emit('basket:updated', { items: this._items, totalAmount: this._totalAmount });
      this.events.emit('card:delete', product);
  }

  updateTotalAmount(): void {
    this._totalAmount = this._items.reduce((sum, item) => sum + item.price, 0) || null;
  }
  ...
  clearBasket(): void {
    ...
    this.events.emit('basket:cleared', { items: this._items, totalAmount: this._totalAmount });
  }
}
